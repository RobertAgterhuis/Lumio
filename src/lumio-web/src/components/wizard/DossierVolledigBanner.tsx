...
export function DossierVolledigBanner({ className }: { className?: string }) {
  ...
  return (
    <ScaleIn show={visible} duration={400} initialScale={0.9}>
      <div
        role="status"
        aria-live="polite"
        aria-atomic="true"
        ...
        className={cn(
          "flex items-center gap-3 rounded-xl border border-success/20 bg-linear-to-r from-success-100 via-success-100/95 to-sage-100/60 px-5 py-4 text-success shadow-md ring-1 ring-success/10",
          className,
        )}
      >
        ...
        <PartyPopper className="h-5 w-5 shrink-0 opacity-60" aria-hidden="true" />
      </div>
    </ScaleIn>
  );
}
