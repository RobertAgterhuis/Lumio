    ...
    <section className="py-20 bg-(--color-primary-50)">
      <Container>
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          ...
          <div className="grid grid-cols-2 gap-5">
            {PRIVACY_POINTS.map((item) => (
              <div key={item.title} className="bg-white rounded-xl p-5 border border-primary-100">
                <item.icon className="w-6 h-6 mb-3 text-primary-700" strokeWidth={1.5} />
                <h4 className="font-semibold text-(--color-neutral-900) mb-1">{item.title}</h4>
          ...
          {/* Text */}
          <div>
            <span className="inline-flex rounded-full bg-primary-100 text-primary-700 px-3 py-1 text-xs font-semibold tracking-wide mb-4">
              Privacy by design
            </span>
            <h2 className="font-display text-3xl md:text-4xl text-(--color-neutral-900) mb-5 leading-tight">
