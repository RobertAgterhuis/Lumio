      ...
      <Container>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {[
            ...
            <div key={item.label} className="bg-white rounded-xl p-6 border border-primary-100 text-center">
              <p className="text-xs font-semibold text-neutral-600 uppercase tracking-widest mb-2">
                {item.label}
              </p>
              <p className="font-display text-2xl text-primary-700 mb-1">{item.value}</p>
        ...
        {/* Scale table */}
        <div className="bg-white rounded-2xl overflow-hidden border border-primary-100">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-primary-100">
              {SCALE_TIERS.map((tier) => (
                <tr key={tier.users} className="hover:bg-(--color-primary-50) transition-colors">
                  <td className="px-6 py-4 text-(--color-neutral-900) font-medium">{tier.users} medewerkers</td>
                  <td className="px-6 py-4 text-right text-(--color-neutral-900)">€{tier.total.toLocaleString("nl-NL")}</td>
