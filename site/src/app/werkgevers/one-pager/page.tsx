...
export default function OnePagerPage() {
  return (
    <div className="bg-white print:bg-white">
      ...
      <div className="print:hidden bg-[var(--color-primary-50)] border-b border-[var(--color-primary-100)] py-3">
        <Container>
      ...
      <div className="py-12 print:py-8">
        <Container>
          {/* Header */}
          <div className="flex items-start justify-between gap-8 mb-10 pb-8 border-b border-[var(--color-primary-100)]">
            <div className="flex-1">
              <div className="inline-flex rounded-full bg-[var(--color-primary-100)] text-[var(--color-primary-700)] px-3 py-1 text-xs font-semibold tracking-wide mb-4">
                Voor werkgevers — Lumio
              </div>
              <h1 className="font-display text-4xl print:text-3xl text-[var(--color-neutral-900)] leading-tight mb-3">
                Geef uw medewerkers de rust<br />die ze verdienen.
              </h1>
              <p className="text-lg text-[var(--color-neutral-600)] max-w-xl leading-relaxed">
            ...
            <div className="shrink-0 text-right hidden md:block print:block">
              <div className="inline-block bg-[var(--color-primary-700)] text-white rounded-2xl px-8 py-6 text-center">
          ...
          <div className="grid md:grid-cols-2 gap-10 mb-10">
            ...
            <div>
              <h2 className="font-display text-xl text-[var(--color-neutral-900)] mb-5">
              ...
              <div className="space-y-5">
                {DIFFERENTIATORS.map((d) => (
                  <div key={d.title} className="flex gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-[var(--color-neutral-900)] mb-0.5">{d.title}</p>
                      <p className="text-sm text-[var(--color-neutral-600)] leading-relaxed">{d.body}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <ul className="space-y-3 mb-8">
                {HR_USE_CASES.map((uc) => (
                  <li key={uc} className="flex gap-3 text-sm text-[var(--color-neutral-600)]">
                    <span className="text-[var(--color-sage-600)] font-bold shrink-0">→</span>
                    <span>{uc}</span>
                  </li>
                ))}
              </ul>
              ...
              {/* Pricing table */}
              <h2 className="font-display text-xl text-[var(--color-neutral-900)] mb-4">
                Schaalvoordelen
              </h2>
              <table className="w-full text-sm border border-[var(--color-primary-100)] rounded-xl overflow-hidden">
                ...
                <tbody className="divide-y divide-[var(--color-primary-100)]">
                  {SCALE_TIERS.map((tier) => (
                    <tr key={tier.users}>
                      <td className="px-4 py-2.5 font-medium text-[var(--color-neutral-900)]">
                        {tier.users} medewerkers
                      </td>
                      <td className="px-4 py-2.5 text-right text-[var(--color-neutral-900)]">
                        €{tier.total.toLocaleString("nl-NL")}
                      </td>
                      <td className="px-4 py-2.5 text-right text-[var(--color-neutral-600)]">
                        €{PRICE_PER_USER}
                      </td>
                    </tr>
                  ))}
          ...
          {/* WKR callout */}
          <div className="bg-[var(--color-primary-50)] border border-[var(--color-primary-100)] rounded-2xl p-6 mb-10">
            <div className="grid md:grid-cols-3 gap-6 text-center">
              {[
                ...
                <div key={item.label}>
                  <p className="text-xs font-semibold text-[var(--color-primary-600)] uppercase tracking-widest mb-1">
                    {item.label}
                  </p>
                  <p className="font-display text-xl text-[var(--color-primary-700)] mb-0.5">{item.value}</p>
                  <p className="text-xs text-[var(--color-neutral-500)]">{item.note}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Footer / CTA */}
          <div className="flex items-center justify-between gap-6 pt-6 border-t border-[var(--color-primary-100)]">
            <div>
              <p className="font-semibold text-[var(--color-neutral-900)] mb-0.5">Interesse? Vraag een gratis pilot aan.</p>
              <p className="text-sm text-[var(--color-neutral-600)]">
          ...
          {/* Legal footer */}
          <p className="text-xs text-[var(--color-neutral-400)] mt-6 print:mt-4">
