...
- [ ] If cycle_type is SCOPE_CHANGE: all stories in the affected dimension that are no longer executable are tagged `SCOPE_CHANGE_HOLD SC-[N]` or `SCOPE_CHANGE_CANCELLED SC-[N]`; REQUEUED stories carry restored status; unaffected stories carry original status (or `NOT_APPLICABLE — normal audit cycle`)
