...
| DEC-115 | HOOG | Alle sprints — UX, Theming | Theme default is ALTIJD light — geen OS `prefers-color-scheme` fallback | Gebruiker is de enige die dark mode mag activeren via Instellingen. Theme wordt opgeslagen in `localStorage("lumio-theme")`; bij afwezigheid is de default `"light"`. `public/theme-init.js` en `src/hooks/useTheme.ts` zijn aangepast: alle `window.matchMedia("(prefers-color-scheme: dark)")` fallback-logica is verwijderd. Reden: voorspelbaar gedrag voor nieuwe gebruikers; dark mode is opt-in. Commit: `3d1cdce`. | 2026-03-05 |
| DEC-116 | HOOG | Alle sprints — i18n, Architecture | Alle domain layout.tsx bestanden MOETEN alle namespace JSON-files importeren die door child-componenten worden gebruikt | next-intl v4 vereist dat namespace JSON handmatig wordt gemerged in `DomainMessagesProvider`. Als een layout een namespace niet importeert, tonen componenten rauwe vertaalsleutels in plaats van vertaalde tekst. Elk nieuw domein of nieuw component dat een extra namespace gebruikt, vereist een update van het bijbehorende `layout.tsx`. Volledige audit: `devdocs/translation-keys-audit.md`. Commit: `1b5823a`. | 2026-03-05 |
| DEC-117 | HOOG | Alle sprints — i18n, UX | Geen hardcoded UI-zichtbare strings — alle tekst via `t()` (next-intl) | Alle gebruiker-zichtbare tekst in `src/lumio-web/` (labels, aria-labels, placeholders, tooltips, alt-tekst) moet via de `useTranslations()` hook en `t()` functie uit next-intl komen. Enige uitzondering: `global-error.tsx` (rendert buiten alle providers — hardcoded NL fallback toegestaan). Vertaalsleutels worden onderhouden in `src/lumio-web/messages/{nl,en}/`. Commit: `d31cec5`. | 2026-03-05 |
| DEC-118 | HOOG | Alle sprints — UX, Layout | Domain page header layout: VoorbeeldDialog + SectieNotitie ALTIJD in aparte utility row onder de header | Op alle domain pages (testament, euthanasie, donor, uitvaart, digitaal-bezit, boedel, documenten, erfgenamen, noodcontacten) zijn VoorbeeldDialog en SectieNotitie verplaatst van de titel-`<div>` naar een aparte `<div className="flex items-center gap-4">` utility row direct onder de paginaheader. Dit voorkomt dat de header visueel opgeblazen wordt. Nieuwe domain pages MOETEN dit patroon volgen. Commits: `90cf49b`, `b47b5b6`, `88bf719`. | 2026-03-05 |
| DEC-119 | HOOG | Alle sprints — i18n, Architecture | i18n gebruikt UITSLUITEND SPLIT file structuur — monolithic files VERWIJDERD | **Update 2026-03-05:** Monolithic `messages/nl.json` en `messages/en.json` volledig verwijderd + `scripts/merge-messages.ts` verwijderd. Alle 11 Storybook stories refactored om split files te gebruiken via `src/lib/test-utils/storybook-messages.ts` helper. **Single Source of Truth:** alleen `messages/{nl,en}/*.json` split files bestaan nog. Root messages worden geladen uit split files (shared.json, ui.json, auth.json, dashboard.json) door `src/i18n/request.ts`. Nieuwe translation namespaces MOETEN worden toegevoegd aan de juiste split file: **(A) Algemene keys** (beschikbaar overal): shared.json namespace `common`, `contacts`, `nav`, `enums`, etc. **(B) Domain keys** (per route): domein-specifiek JSON bestand (testament.json, eigenaar.json, uitvaart.json, erfgenamen.json, etc.). **(C) Storybook**: voeg import toe aan `storybook-messages.ts` indien nieuwe namespace nodig is. Loader comment in `src/i18n/request.ts` MOET worden bijgewerkt bij namespace wijzigingen. Root namespaces (altijd beschikbaar): shared (common, nav, enums, feedback, errors, idle, verwijderBevestiging, sectieNotitie, domainStatus, search, shortcuts, wizard, contacts), ui (personSelect, help, hulpteksten, legeStaten), auth, dashboard. Domain namespaces (per-route): boedel, testament, uitvaart, eigenaar, donor, euthanasie, digitaal-bezit, documenten, erfgenamen, noodcontacten, videoboodschappen, instellingen, export, misc. **Rationale:** DIT BESLUIT VOORKOMT BUG-I18N-001 + elimineert verwarring tussen "waar voeg ik keys toe?" door monolithic build artifacts volledig te verwijderen. Clean Code principe: expliciete imports, Single Source of Truth. | 2026-03-05 |
| DEC-120 | HOOG | Alle sprints — Code Quality, Architecture | Clean Code constraints: design tokens, type safety, translation keys, component patterns | **(1) Design tokens**: Gebruik ALLEEN design tokens uit `messages/{nl,en}/shared.json > enums` en `src/lib/design-tokens.ts`. Nooit raw Tailwind color classes (`bg-blue-600`), altijd semantische kleuren (`bg-primary-600`, `text-danger-700`) of token variables. **(2) Type safety**: Alle API calls moeten typed zijn (response/request types). Gebruik `SharedContact`, `ContactType` enums, nooit any-types. **(3) Translation keys**: Alle gebruiker-zichtbare tekst via `t()` (useTranslations) — zie DEC-117. Translation keys volgen namespace.key pattern (`contacts.name`, `common.cancel`). **(4) Component patterns**: Generieke componenten (zoals ContactSelector) hebben NO domain-coupling en accepteren contactType als prop; gebruiken shared messages namespaces (common, contacts), NOOIT domain-specific namespaces (testament, eigenaar). Domain components gebruiken DomainMessagesProvider in layout.tsx voor domain namespaces. **(5) Data normalisatie**: Geen data duplicatie — gebruik SharedContact met type discriminator patroon (FK verwijzingen, geen embedded kopieën van contactgegevens). **(6) Error handling**: Alle async operaties hebben try/catch met gebruiker-zichtbare feedback (AlertDialog, toast, etc.). Formateer API-validatiefouten per-veld via `ApiError.getFieldErrors()` en toon gebruiker-vriendelijk met veldlabels + format-hints. **(7) Async side effects + cache invalidation**: Bij auto-save/sync flows (bijv. contact-select → auto-post naar noodcontacten) MOET na succesvolle API-call direct `queryClient.invalidateQueries()` worden aangeroepen voor de betreffende domain (bijv. `{ queryKey: domainKeys.all("noodcontacten") }`). Dit voorkomt dat gebruikers handmatig moeten refreshen om wijzigingen te zien. Pattern: plaats invalidatie in useEffect DIRECT na POST/PUT, voordat de async functie eindigt. Rationale: Deze constraints zorgen voor maintainable, accessible, type-safe code en voorkomen regressies zoals BUG-I18N-001 (vertaalsleutels niet geladen), data duplicatie (Phase 1-6 eliminations), design token violations (SP-UX-01 findings), en manual refresh bugs (like noodcontacten not appearing after contact select). | 2026-03-05 |
