...
| DEC-105 | KRITIEK | SP-8 / Fase 3 | OnboardingWizard focus-trap MOET worden geïmplementeerd via `dialog.tsx` refactor of eigen Tab-trap | REEVALUATE FASE-3 (RP-ACC-002 carryover): SP-UX-01 heeft `aria-modal="true"` toegevoegd maar geen keyboard-trap. `dialog.tsx` heeft een correcte focus-trap, maar `OnboardingWizard` gebruikt een custom `<div>` in plaats van het `DialogContent`-component. WCAG SC 2.1.1 is daardoor nog steeds geschonden. Besluit: refactor `OnboardingWizard` naar `<DialogContent>` OF kopieer Tab-interceptie-logica naar de `useEffect` op `modalRef`. Tracked als story SP-8-UX-001. | 2026-03-02 |
| DEC-106 | HOOG | SP-8 / Fase 3 | `IdleWarningDialog` moet worden gewrapped in `<DialogContent>` | REEVALUATE FASE-3 (UI-001 / RP-ACC-NEW-01): `IdleWarningDialog` rendert `DialogHeader` + `DialogFooter` als directe children van `<Dialog>` zonder `<DialogContent>`-wrapper. Gevolg: geen portal, geen backdrop, geen focus-trap, geen `role="dialog"`. Besluit: voeg `<DialogContent>` toe als wrapper. Tracked als story SP-8-UX-002. | 2026-03-02 |
