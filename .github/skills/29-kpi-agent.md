...
On first activation (after Synthesis Final Report, before sprint 1) the agent reads the KPI baseline from the Synthesis Final Report and writes it to `docs/metrics/kpi-baseline.json`:
...
Read `docs/metrics/kpi-baseline.json`. If the file does not exist: perform Step 0 first.
...
**For `OFF_TRACK` over 2+ consecutive sprints: mandatorily write a `LESSON_CANDIDATE`** to `docs/retrospectives/lessons-learned.md` per RULE ORC-22 (type: `KPI_MISS`, category: `VELOCITY` or `BLOCKER` depending on the KPI). Include the number of consecutive sprints and the trend in the description.

**For `brand_violations_count` > 0 over 2+ consecutive sprints: mandatorily write a `LESSON_CANDIDATE`** to `docs/retrospectives/lessons-learned.md` (type: `BRAND_MISS`, category: `BRAND_COMPLIANCE`). Include the sprint IDs, total number of violations and most common violation type (color, typography, logo or tone of voice).
