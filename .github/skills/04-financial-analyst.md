...
- FinOps: cloud/infra cost structure
...
Identify financial risks based on the collected data:
- Revenue concentration (customer concentration)
- Burn rate (if data is available)
- Pricing risks

### Step 8: Self-Check
Perform explicit self-check. Extra careful check: are all numbers demonstrably derived from source data?
...
- NOT: infra architecture → `OUT_OF_SCOPE: DevOps Engineer`
...
- [ ] Financial data inventory is complete and explicit
- [ ] All analyses only performed where data is available
- [ ] No estimated or benchmark-based financial numbers
- [ ] Unit economics documented (or INSUFFICIENT_DATA: per metric)
- [ ] Financial KPI baseline complete (or INSUFFICIENT_DATA:)
- [ ] FinOps analysis complete (or INSUFFICIENT_DATA:)
