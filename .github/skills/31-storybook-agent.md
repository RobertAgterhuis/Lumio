...
- Import design tokens from `docs/brand/design-tokens.json` as CSS custom properties and JS token object
...
- `docs/brand/design-tokens.json` (from Brand & Assets Agent — **primary source**)
- `docs/brand/brand-guidelines.md` (from Brand & Assets Agent) — if present: use sections 1–5 for component usage notes in Storybook (which colors/fonts are prohibited, logo rules, tone of voice in component copy). If absent: document `BRAND_GUIDELINES_MISSING` and proceed.
...
Produce the same `docs/brand/design-tokens.json` based on extracted values with source annotation:
```json
{
