...
2. **Business Document Generation** — After questionnaire answers are processed (on AUDIT or REEVALUATE), this agent transforms accumulated answers into authoritative, reusable official documents stored in `BusinessDocs/OfficialDocuments/`. These documents evolve with every cycle and represent the living record of the business, brand, and solution.
...
**Trigger (document generation):** Called by the Orchestrator after questionnaire answers have been loaded at the start of an AUDIT or REEVALUATE cycle and after phase agents complete their work.
...
**Domain-specific guardrails:** `.github/guardrails/09-questionnaire-guardrails.md`  
**Output contract:** `.github/contracts/questionnaire-output-contract.md`
...
Next step: Customer fills in questionnaires → REEVALUATE or new AUDIT to consume answers
```
...
This workflow is triggered by the Onboarding Agent at the START of every AUDIT or REEVALUATE cycle.

### Step 0: Cross-cycle staleness check (new AUDIT only — skip for REEVALUATE)
...
| NEW minor or patch version (e.g., v1.1 → v1.2) | Load all answers. In the answer map, set `answer_age_status: "PRIOR_CYCLE"` for each entry. Phase agents receive a warning prefix in their context block: `⚠️ These answers were collected in a prior audit cycle. Verify that they still apply to the current version.` |
...
[⚠️ PRIOR_CYCLE: These answers were collected in a prior audit cycle — verify they still apply.]
...
When a new AUDIT cycle is run and the questionnaire files contain answers from a previous cycle:
...
Full schema: `.github/contracts/questionnaire-output-contract.md`  
Guardrails: `.github/guardrails/09-questionnaire-guardrails.md`
...
### Universal
- [ ] Output complies with .github/contracts/questionnaire-output-contract.md
- [ ] No G-QST-01 through G-QST-10 violations unresolved
- [ ] No analysis or recommendations added to outputs (G-QST-10)
